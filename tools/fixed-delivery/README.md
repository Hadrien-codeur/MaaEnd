...
最新实机结果：连滑首轮到末架后仍等待第 15 次 E，实际日志只有 14 次 E，30 秒后自行超时。修复方向为首跳左键后 14 次 E；当前安装版尚未改预算，详见 [连滑首轮日志分析](../../docs/zh_cn/dev-notes/苏白易连滑首轮日志分析.md)。下面的 15 次规则描述当前实现，不代表已通过连滑实机验收。
