...
### 规矩 3：滑索 `target` ≠ 起点架，架数要说清含不含起点

2026-08-08 三条送货路线因这条全挂（案例见规范 §11.1）。两点：

**其一，`MapTrackerZipline.target` 是「第一跳飞向的下一架」，不是脚下这架。** 脚下那架是 `HEADING` 的目标（正对它才好上索），两者是不同的点：

| 参数 | 指向 | 距站位 |
| --- | --- | --- |
| 起点段 `HEADING.target`（P4 / D0-4） | 脚下这架 | 1~2 m |
| `MapTrackerZipline.target`（P5 / D1） | **下一架** | 通常 30~80 m |

录 D1 时人还站在起点架旁，**准星要指向远处那一架**。算出来 < 5 m 就是录错了。

**其二，报架数请写「共 N 架（含起点架）」。** 差一个起点架，`chain_max_press` 就差 1，人会落错架子：

```
chain_max_press = 跳数 − 1 = 含起点架的架数 − 2
```
...
| P5 | 发射要瞄准的**下一架**滑索架 ⚠️规矩3（距站位应 30~80m） | `MapTrackerZipline.target` | `[    ,     ]` |
| P6 | 滑索架总数（**含**起点架）⚠️规矩3 | `chain_max_press` = 总数 − 2 | ____ 架（含起点） |
...
| # | 录什么 | 用途 | 值 |
| --- | ------ | ---- | --- |
| D1 | 发射瞄准的**下一架**滑索架 ⚠️规矩3（距站位应 30~80m） | `MapTrackerZipline.target` | `[    ,     ]` |
| D2 | 滑索架总数（**含**起点架）⚠️规矩3 | `chain_max_press` = 总数 − 2 | ____ 架（含起点） |
