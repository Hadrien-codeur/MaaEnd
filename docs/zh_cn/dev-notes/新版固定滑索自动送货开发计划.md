...
2026-09-14 更新：补强版苏白易一轮空跑及日志核验通过。博士已确认 #0～#15 整段连滑，途中不判断下一架位置，段末仍确认实际落点；真实业务和其他路线继续暂缓。经首轮实测发现多等一次提示，博士已确认修正：首跳由左键完成，之后再按 N−2 次 E，本路线为 14 次。当前已实现连滑，软件检查通过，待整段实机试跑。以下历史接力数推导不替代最新确认，详见 [实机验收记录](./苏白易固定逐跳实机验收记录.md)。
