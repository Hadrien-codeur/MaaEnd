...
- `chain_max_press`: 非负整数，默认 `0`。发射后在飞行途中按「长距滑索」接力键（E）以**不落地连滑**到下一架的最大次数。`0` 表示不连滑，保持"逐架落地重跳"的经典行为；当 `N>0` 时，途中每检测到 E 提示就按，累计到 N 次后停止，玩家落在第 `N+1` 架（终点架的 E 提示因已达上限不再按下）。识别与按键复用 `RealTimeAutoZipline` 节点（右侧 ROI 的长距滑索图标）。
