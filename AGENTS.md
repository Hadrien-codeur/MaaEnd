...
涉及本定制项目时，先读 [接力记录](docs/zh_cn/dev-notes/新版固定滑索送货接力记录.md)、[项目开发规范](docs/zh_cn/dev-notes/新版固定滑索送货开发规范.md)，再读 [已确认开发计划](docs/zh_cn/dev-notes/新版固定滑索自动送货开发计划.md)。

- 用户已于 2026-09-08 确认开始，在独立新版仓库推进 P0；不能将环境准备等同于实机验收通过。
- 公司新版根目录为 `E:\MaaEnd V2`。旧 `E:\TestBase2\MaaEnd` 只读参考，不作为构建、安装、写入或同步目标。
- 开发分支为 `codex/fixed-zipline-delivery`；`origin` 读取上游，`myfork` 指向 `Hadrien-codeur/MaaEnd`。用户已授权将本项目代码和文档提交、推送到个人分支进行双电脑接力。
- 每次接力先检查本地改动和远端进度，再同步子模块、依赖与构建；每次收尾更新记录并核实推送结果。账号配置、原始日志和构建缓存不提交。
- 延续已确认的 AutoDelivery / MapNavigator 方案，不恢复 MapTracker，不整体合并旧功能分支；按阶段解释改动和验收情况。
- 每次对话称呼用户为“博士”（《明日方舟》主角的称呼）。
