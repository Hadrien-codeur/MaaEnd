...
2026-09-18：材料研究所、观测站、猫头鹰已按在线填写接入抢单／装箱自送的固定分发，并新增独立空跑入口。支持固定链专用 `fixed_approach_path` / `fixed_departure_path` 逐点录制地面段及最终 HEADING；配置中的 `dismount_heading` 仅在末架确认后、下索前闭环转向。三条路线尚待博士实际接单验证，完整架序、入口和自动化结果见 [接入记录](../../docs/zh_cn/dev-notes/武陵城其他三条固定路线接入记录.md)。
