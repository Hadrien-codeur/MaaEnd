...
2026-09-19：材料研究所末架下索后原先从名义落点预规划到录制首点，产生错误绕行；固定路线现从稳定实测落点动态接入首点，无法接入时明确失败。原生规划和自动检查通过，待博士实机空跑，正式交付闭环未验收。材料研究所、观测站、猫头鹰此前已按在线填写接入独立空跑和正式分发；完整架序见 [接入记录](../../docs/zh_cn/dev-notes/武陵城其他三条固定路线接入记录.md)，当轮修复与结果见 [接力记录](../../docs/zh_cn/dev-notes/新版固定滑索送货接力记录.md)。
