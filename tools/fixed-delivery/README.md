...
2026-09-20：博士反馈武陵城四个、试验园区三个送货终点已在抢委托送货与转交委托两类正式入口实际通过；七个终点均已生成 `fixed_route_node` 并接入共享 AutoDelivery。当前冻结路线接口，准备上游版本兼容迁移；异常矩阵仍单独验收。

2026-09-19：材料研究所末架下索后原先从名义落点预规划到录制首点，产生错误绕行；固定路线现从稳定实测落点动态接入首点，无法接入时明确失败。博士已确认材料研究所修复版空跑正常。苏白易新增录制离索段，武陵城四个送货终点共用该接入机制；接单实测任务支持选择四终点。本轮博士反馈实测没有问题，最近日志确认材料研究所真实接单固定导航成功、接单任务成功退出；其他三终点本轮逐项交付及装箱自送／异常矩阵仍未单独核验。下一阶段准备试验园区，完整架序见 [接入记录](../../docs/zh_cn/dev-notes/武陵城其他三条固定路线接入记录.md)，最新状态见 [接力记录](../../docs/zh_cn/dev-notes/新版固定滑索送货接力记录.md)。
...
- 生成源在 `tools/pipeline-generate/AutoDelivery/routes.json`；固定 ID 只传给新增的 WithFixedZipline 主路线，不进入普通 WithZipline、步行或站位修正节点。运行时目录的 `fixed_route_node` 供共享 Go 分发选择；当前包含武陵城固定取货、武陵城四个送货终点和试验园区三个送货终点。
