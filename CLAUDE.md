...
2. **上下文提醒**：留意本会话的上下文用量，当估计接近 **500k tokens** 时，主动提醒博士"当前上下文已接近 500k，注意及时整理/开新会话或压缩"。
    - ✅ 已配置自动 hook（会话 10）：`UserPromptSubmit` 事件触发 `.claude/hooks/context_reminder.py`，每次博士发消息时读 transcript 估算上下文，越过 500k/700k/900k 档位时各提醒一次（systemMessage）。配置在 `.claude/settings.json`。
    - ⚠️ 该 hook 用 transcript 字符数 ÷ 3 估算 token，**是粗略估算非精确值**，可能偏早或偏晚；Claude 自己也应"尽量记得"作为兜底。
    - hook 脚本与 settings.json 已入 git（多电脑同步）；`.cache/` 标记目录和 `settings.local.json` 不入仓。
...
## 20. 会话 9（v2.3 落地）：ColorMatch + 路线 B roi + 空跑验证节点

> 本节回写会话 9 实际落地的代码（已 commit `5f67b5cd`，文档之前漏记）。

### 20.1 本会话改动（SelfDeliver.json）

1. **`CheckRouteOriginiumScienceParkA` 由 OCR 改 ColorMatch**（兑现 19bis.3）：
    - `recognition: "ColorMatch"`，`lower:[200,90,0]` `upper:[255,180,80]` `connected:true` `count:30`（**均为估计值，待真实截图像素调**）
2. **新增路线 B 节点** `CheckRouteOriginiumScienceParkB`（同 ColorMatch 参数，roi 不同）
3. **A/B 两条路线 roi 量定并写死**（取自会话 7/9 量图，2560×1440 → 1280×720，显示宽 2000 ×0.64）：
    - 路线 A 送货点中心 (725, 371) → `roi: [676, 348, 100, 46]`（y 区间 348–394）
    - 路线 B 送货点中心 (655, 522) → `roi: [604, 500, 104, 46]`（y 区间 500–546）
    - 两区间留 ~106px 缓冲带，**不重叠** ✅（解决会话 7/8 的卡点）
4. **`MapDispatch.next`** 改为 `[CheckRouteA, CheckRouteB, NoRouteMatch]`
5. **新增空跑验证临时节点** `VerifyRouteHit` / `StopForVerify`：
    - CheckRouteA/B 命中后，`next` **暂时**指向 `VerifyRouteHit`（ESC 关地图）→ `StopForVerify`（StopTask）
    - 目的：填入真 roi 后第一次脱离零道具安全网前，先确认 ColorMatch 落点分发正确（A 图命中 CheckRouteA、B 图命中 CheckRouteB，不误中橙色传送点图标）
    - **验证通过后要做**：删除 VerifyRouteHit / StopForVerify 两节点，把 CheckRouteA/B 的 `next` 改回 `["DeliveryJobsSelfDeliverCloseMapAndStartDelivery"]`（恢复正式接单流程）

### 20.2 当前 SelfDeliver.json 节点全貌（会话 9 后）

```
ClickViewLocation (OCR查看位置→Click, roi已填[1040,150,140,50])
  → MapDispatch (DirectHit路由)
      ├ CheckRouteOriginiumScienceParkA (ColorMatch橙色, roi[676,348,100,46], anchor RouteA)
      │     → VerifyRouteHit (ESC) → StopForVerify (StopTask)   ← 空跑验证临时接线
      ├ CheckRouteOriginiumScienceParkB (ColorMatch橙色, roi[604,500,104,46], anchor RouteB)
      │     → VerifyRouteHit → StopForVerify                     ← 同上
      └ NoRouteMatch (ESC) → StopForNoRouteMatch (StopTask, 文案提示)
  [正式流程被验证节点临时替代] CloseMapAndStartDelivery → RedistributionBidNextStep
  PickupStart (InWorld→[Anchor]PickupEntrance / NoRoute)
  NoRoute / End
```

### 20.3 仍未做的 TODO（不变）

- `SelfDeliver/OriginiumSciencePark.json` 路线 A 内部全 TODO（zone_id / target / path / 取货动作 / 交货动作）→ 阶段 4-5，等 MapNavigator 录制
- 路线 B 的取货/送货流程文件**尚未建**（CheckRouteB.anchor 指向的 `...RouteBPickupStart` 节点还不存在；空跑验证阶段不接单不走 anchor，不影响验证）
- ColorMatch 的 `lower/upper/count` 待真实截图像素调

---

## 21. 会话 10（2026/06/08）：同步上游（游戏 1.3 更新修 bug）

### 21.1 起因

用户准备跑空验证时，**游戏更新到 1.3 导致前置流程卡 bug**。决定先拉官方上游更新再跑。

### 21.2 已完成的 git 同步 ✅

- `git fetch origin v2` → 上游领先 **223 个提交**
- `v2` 分支 fast-forward 到 `origin/v2`（`725cd130`）
- `feature/self-deliver-route` **零冲突合并** v2（merge commit `ff101c68`）
- 子模块随上游更新（`assets/resource/model` → `8acf54f`，`tests/MaaEndTestset` → `3025b3b`）
- 「自己送货」全部代码完整保留，未被上游触碰（上游仅动 DeliveryJobs 区域 1 个无关文件 #3405）

### 21.3 ⚠️ 关键：install/agent 二进制需更新（疑似卡 bug 真因）

- `install/` 下 `resource`/`tasks`/`locales`/`data` 都是**软链接** → pipeline JSON / i18n 改动**立即生效**
- 但 `install/agent/` 是**真实目录**，里面 `cpp-algo.exe`/`go-service.exe` 是 **4 月 22 日旧版**（本地 `version.json`: maafw v5.10.2 / mxu v2.1.1 / cpp_algo v2.7.0-beta.5）
- 上游这 223 提交含 **49 个 agent/ 代码提交**，关键的有：
    - `feat(AutoStockpile): 适配1.3 (#3403)` ← **游戏 1.3 适配**
    - `fix(MapNavigator): NavMesh 可能错误越过临近不可通行区域 (#3418)`
    - `feat(map): 为MapTracker引入NavMesh功能 (#3359)` 等一系列导航/定位修复
- **结论**：用户的卡 bug 很可能是「pipeline 软链接已最新，但跑识别/导航的 exe 还是旧版、不认 1.3 逻辑」。
- **修复方式**：用户的 agent exe 来自**下载官方 release 包**（不本地编译）。需下载与上游代码版本匹配的最新 release（最新 tag 见会话 10 时 `v2.10.0-beta.3`），把里面的 `cpp-algo.exe`/`go-service.exe`（及 maafw/mxu 若有更新）覆盖到 `install/agent/`，并更新 `install/version.json`。

### 21.4 push 被拒 → 已解决 ✅（PAT 加 workflow scope）

- 现象：`git push myfork v2` / `feature` 被 GitHub 拒绝：
  `refusing to allow a Personal Access Token to create or update workflow .github/workflows/check-submodule.yml without workflow scope`
- 原因：上游 223 提交改了 `.github/workflows/` 下的文件，PAT 没有 `workflow` 权限
- 解决（博士已操作）：GitHub Settings → Developer settings → Tokens → 给 PAT 勾上 `workflow` scope → 重新 push 成功
    - `v2`: `725cd130..2ad5007e`
    - `feature/self-deliver-route`: `5f67b5cd..ff101c68`

### 21.5 ⚠️ 重要认知纠正：git clone ≠ 拿到 exe

- **`git pull`/`clone` 永远不会给你 `cpp-algo.exe`/`go-service.exe`/`MaaEnd.exe`**——仓库里只有源码，没有编译产物。
- exe 两种来源：① 本地编译（cpp-algo 需装 ~10GB 的 Visual Studio + CMake；本机只有 Go，无 VS/CMake/ninja）② **下载官方 Releases 成品 zip**（推荐）。
- `install/` 整个目录**不在 git 跟踪里**，由 `tools/build_and_install.py` 生成（assets 软链接 + go build + 可选 cpp-algo 编译）。
- 博士选择"下载官方 release"，见 21.6。

### 21.6 二进制更新实操（会话 10 已完成 ✅）——以后游戏更新/换电脑照此做

官方最新正式版 **v2.13.0**（2026-06-05，含 1.3 适配）。Windows 成品包：`MaaEnd-win-x86_64-v2.13.0.zip`（199 MB）。
查最新版本与资源名：

```bash
curl -sL "https://api.github.com/repos/MaaEnd/MaaEnd/releases/latest" -o rel.json
python -c "import json;d=json.load(open('rel.json',encoding='utf-8'));print(d['tag_name']);[print(a['name']) for a in d['assets']]"
```

下载页：https://github.com/MaaEnd/MaaEnd/releases/latest → Assets → `MaaEnd-win-x86_64-*.zip`

**⚠️ 关键原则：只覆盖二进制，绝不动软链接目录。**

| 从 zip 复制                                                             | 覆盖到                                      | 说明                                      |
| ----------------------------------------------------------------------- | ------------------------------------------- | ----------------------------------------- |
| `agent/cpp-algo.exe`                                                    | `install/agent/cpp-algo.exe`                | C++ 识别器（MapLocator/NavMesh/格子识别） |
| `agent/go-service.exe`                                                  | `install/agent/go-service.exe`              | Go 识别器/动作                            |
| `agent/WebView2Loader.dll`                                              | `install/agent/WebView2Loader.dll`          | v2.13 新增                                |
| `MaaEnd.exe`                                                            | `install/MaaEnd.exe`                        | **主程序已改名**（旧版叫 `mxu.exe`）      |
| `maafw/*`（逐文件，保留目标独有的 `MaaNode.node`/`MaaNodeServer.node`） | `deps/bin/`（= `install/maafw` 软链接目标） | MaaFramework 全套 dll，v2.13 也更新了     |

**不要覆盖**：`install/` 下的 `resource`/`tasks`/`locales`/`data`/`resource_*` —— 这些是软链接，指向带「自己送货」改动的 `assets/`，覆盖即丢功能。
`interface.json` 用**仓库自身的 `assets/interface.json`**（git 已随上游更新）覆盖 install 旧副本，**不要**用 zip 里的。

会话 10 实际执行：

1. 备份旧二进制到 `install/_backup_pre_v2.13/`（cpp-algo/go-service/mxu.exe/version.json + deps/bin 整份）
2. 覆盖 agent 3 文件、maafw 全套（md5 校验 MaaFramework.dll 已换新、补上 `MaaAgentBinary`、保留 `.node`）
3. 放入新 `MaaEnd.exe`、用 `assets/interface.json` 刷新 install/interface.json、改写 `version.json` 标注 v2.13.0
4. 验证：软链接完好、SelfDeliver.json 27 处命中仍在、所有二进制时间戳 6/5

**遗留**：旧 `install/mxu.exe`（4/22）暂留，待博士确认新 `MaaEnd.exe` 能启动后删除。

### 21.7 下次接力第一步

1. 博士启动 `install/MaaEnd.exe`（**不再是 mxu.exe**）确认能正常打开
