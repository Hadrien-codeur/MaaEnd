...
2026-09-14 更新：补强版苏白易一轮空跑及日志核验通过。博士已确认 #0～#15 整段连滑，途中不判断下一架位置，段末仍确认实际落点；真实业务和其他路线继续暂缓。博士进一步明确：首次左键起滑另计，之后再按 N−1 次 E，本路线为 15 次。当前已实现连滑，软件检查通过，待整段实机试跑。以下历史接力数推导不替代最新确认，详见 [实机验收记录](./苏白易固定逐跳实机验收记录.md)。
...
2026-09-14 计数约定已由博士修正：一段有 N 架，包含起点和末架；首次左键起滑不计入 E 预算，之后按 N−1 次 E。旧的“架数减二”推导不再用于本项目。预算由已确认的连滑区间派生，而非由运行时逐架定位推进。
