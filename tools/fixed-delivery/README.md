...
2026-09-19：材料研究所末架下索后原先从名义落点预规划到录制首点，产生错误绕行；固定路线现从稳定实测落点动态接入首点，无法接入时明确失败。博士已确认材料研究所修复版空跑正常。苏白易新增录制离索段，武陵城四个送货终点共用该接入机制；接单实测任务支持选择四终点，完整交付闭环仍待博士测试。完整架序见 [接入记录](../../docs/zh_cn/dev-notes/武陵城其他三条固定路线接入记录.md)，本轮状态见 [接力记录](../../docs/zh_cn/dev-notes/新版固定滑索送货接力记录.md)。
