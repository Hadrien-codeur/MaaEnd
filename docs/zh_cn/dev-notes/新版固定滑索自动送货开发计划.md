...
博士要求在真实送货闭环前，先恢复固定取货滑索的路线设计，并引入取货点、交付点与架前两点朝向确认。详见 [固定取货滑索与朝向改造调研方案](./固定取货滑索与朝向改造调研方案.md)。本轮交付为源码对照及待审阅方案；先核验武陵城取货两架、地面通道和交互站位，补地面路径约束、严格朝向结果与转向后站位检查，再做局部无订单验收，随后恢复 P4 业务。该方案没有将拟议字段写入运行资源，不改变苏白易已通过的连滑计数与途中不定位约定。

## 2026-09-13 执行范围修正

2026-09-14 更新：补强版苏白易一轮空跑及日志核验通过。博士已确认 #0～#15 整段连滑，途中不判断下一架位置，段末仍确认实际落点；真实业务和其他路线继续暂缓。经首轮实测发现多等一次提示，博士已确认修正：首跳由左键完成，之后再按 N−2 次 E，本路线为 14 次。当前已完成修复版整段实机试跑：14 次 E、末架确认、自动下索及终点步行全部通过，详见 [连滑实机验收记录](./苏白易连滑实机验收记录.md)。仅确认苏白易正常移动链，不扩大为 P0～P5 全部验收完成。以下历史接力数推导不替代最新确认，详见 [实机验收记录](./苏白易固定逐跳实机验收记录.md)。

博士确认按当前计划推进，并调整：本轮固定逐跳空跑由博士再执行一轮，不采用连续三次门槛；连滑段按具体路线定制，分段待确认；真实业务接入及其他路线暂缓。先补齐记录、Schema、生成入口和离线异常回归。以下 P0～P5 作为完整路线图保留，当前优先级以上述修正及接力记录为准。
...
2026-09-14 计数约定已由博士修正：一段有 N 架，包含起点和末架；首次左键起滑不计入 E 预算，之后按 N−2 次 E。首轮试跑暴露了 N−1 预算多等一次提示，已依据实测和博士确认改为“架数减二”。预算由已确认的连滑区间派生，而非由运行时逐架定位推进。
...
识别上索成功 → 对准首跳并发射 → 确认已起滑
  ...
  → 等待下一次有效提示
  → E 预算用尽后不再按键 → 确认落在预期末架 → 按段定义续行或下索
```
...
**验收：** 苏白易按博士确认直接试跑 #0～#15 整段；其他路线恢复开发后按各自区间验证。覆盖提示持续、漏提示、提前落架、预算用尽后仍出现提示等情形。按 E 次数正确但落错架必须判失败；不能仅凭次数报告成功。
...
1. 本轮苏白易补强后由博士执行一轮独立空跑；结果逐项记录，不据单次成功宣称统计稳定性。其他路线的验收轮次在恢复开发时另行确定。
